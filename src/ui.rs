...
pub fn run(
    ...
    let mut webview = builder()
        ...
        .invoke_handler(|webview, arg| {
            match arg {
                "load_gamelog" => {
                    if let Some(path) = tfd::open_file_dialog(
                        ...
                        sound_tx
                            .send(SoundMessage::ChangeGamelog(path.clone()))
                            .unwrap();
                        gamelog_path.lock().unwrap().replace(path);
                ...
                "load_soundpack" => {
                    if let Some(path) =
                        tfd::select_folder_dialog("Choose soundpack directory", "soundpack")
                    {
                        let path = PathBuf::from(path);
                        sound_tx
                            .send(SoundMessage::ChangeSoundpack(path.clone()))
                            .unwrap();
                        soundpack_path.lock().unwrap().replace(path);
                ...
                "load_ignore_list" => {
                    if let Some(path) = tfd::open_file_dialog(
                        ...
                        sound_tx
                            .send(SoundMessage::ChangeIgnoreList(path.clone()))
                            .unwrap();
                        ignore_path.lock().unwrap().replace(path);
    ...
    Original SoundSense created by zwei:
        http://df.zweistein.cz/soundsense/",
                ...
                "set_default_paths" => {
                    let mut conf_path =
                        dirs::config_dir().expect("Failed to get configuration directory.");
...
fn set_slider_head(webview: &mut WebView<()>, channel_name: &str, is_paused: bool) {
    if is_paused {
        webview
            .eval(&format!(r#"setSliderHead("{}", true)"#, channel_name,))
            .unwrap();
    } else {
        webview
            .eval(&format!(r#"setSliderHead("{}", false)"#, channel_name,))
            .unwrap();
    }
}
